...
def modify_gt_based_on_ad_gq(record, rrecord):
    ...
    for sample_name in record.samples:
        ...
        rads = rsample_info.get('AD', [0, 0]) # Default to [0, 0] if AD is not present
        rref, ralt = rads if len(rads) == 2 else (0, 0)
        rref = 0 if rref is None else rref
        ralt = 0 if ralt is None else ralt
        rdp = 1 if rref + ralt == 0 else rref + ralt
        rgq = rsample_info.get('GQ', 0) # Default to 0 if GQ is not present
        rgq = 0 if rgq is None else rgq
        ...
        if num_hps >= 2 and ralt/rdp >= 0.33:
            rrecord.samples[sample_name]['GT'] = (1, 1)
        ...
        if num_hps >= 3 and ralt/rdp >= 0.30:
            rrecord.samples[sample_name]['GT'] = (1, 1)
            logger.info(f"Setting the GT to (1, 1) due to the num_hps {num_hps} and alt/dp ratio {ralt/(ralt + rref)} for the variant {record.chrom}:{record.pos}:{record.ref} -> {record.alts}")
            continue

        if num_hps >= 4 and ralt/rdp >= 0.25:
            rrecord.samples[sample_name]['GT'] = (1, 1)
            logger.info(f"Setting the GT to (1, 1) due to the num_hps {num_hps} and alt/dp ratio {ralt/(ralt + rref)} for the variant {record.chrom}:{record.pos}:{record.ref} -> {record.alts}")
            continue

        if rgq < 5 and ralt/rdp >= 0.5 and rdp > 5:
            rrecord.samples[sample_name]['GT'] = (1, 1)
...
def sort_vcf(vcf_file, ref_genome, output_vcf = None, tmp_dir = "/tmp", threads = 10, logger = logger):
    ...
    cmd = f'''bcftools norm --threads {threads} -m -both -f {ref_genome} --multi-overlaps 0 -a -Ou {vcf_file} | \
              bcftools norm --threads {threads} -d exact -Ou - | \
              bcftools filter --threads {threads} -i 'ALT[0] != "*" && COUNT(GT="alt") > 0' -Ou - | \
              bcftools sort -T {tmp_dir} -Oz -o {output_vcf} - && \
...
def merge_with_priority(query_vcf = "", 
    ...
    sorted_query_vcf = sort_vcf(query_vcf, ref_genome, tmp_dir = tmpdir, threads = threads, logger = logger)

    # Sort the reference VCF file
    sorted_reference_vcf = sort_vcf(reference_vcf, ref_genome, tmp_dir = tmpdir, threads = threads, logger = logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-fl", "--filter_tag", type=str, help="The filter tag you want to added to the matched query VCF records", required=False, default=None)
    ...
    parser.add_argument('--tmpdir', type=str, default="/tmp", help='Temporary directory for sorting large VCF files', required=False)
