...
def biser(ref_genome: str, build: str, thread: int, output: str):
    ...
    """
    logging.info(f"Extracting SD regions with {thread} threads.")
    logging.info(f"Reference genome asssembly : {build}")
    ...
    if shutil.which("conda"):
        ...
        logging.info(f"Running BISER version {_version}")
...
def main_trim(infile: str, outfile: str, fraglen=300, gaplen=10, logLevel="INFO"):
    ...
    logging.info("****************** CIGAR trimming completed ******************")
...
def annotate_from_refgene(infile, outpath, anno_table: str, genelist: str, keep=False, genecol=16):
    ...
    logging.info("Writing all homo regions to " + os.path.join(outpath, "homologous_regions", "all_homo_regions.bed"))
