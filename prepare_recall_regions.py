...
from src.depth import get_insert_size_distribution
...
def preparation(ref_genome: str,
    ...
    multi_align_bed_fp = os.path.join(outdir, basename + f".{target_tag}.multialign.bed")
    if not os.path.exists(multi_align_bed_fp) or not is_file_up_to_date(multi_align_bed_fp, [input_bam, target_region]):
        if not os.path.isfile(ref_genome + ".fai"):
            raise FileNotFoundError("Reference genome not FAI indexed. ")
        ...
        multi_align_bed.sort().saveas(multi_align_bed_fp)
    else:
        multi_align_bed = pb.BedTool(multi_align_bed_fp)
    ...
    sd_paralog_pairs, connected_qnode_components = extract_SD_paralog_pairs_from_graph( query_nodes, 
                                                                                ...
                                                                                avg_frag_size = avg_insert_size, 
                                                                                std_frag_size = std_insert_size, 
                                                                                threads=threads )
    ...
    # Step 7: Create beds, masked genomes and realign
    get_region_and_realign(grouped_qnode_cnodes,
                                 sd_paralog_pairs,
                                 outdir,
                                 ref_genome,
                                 avg_insert_size = avg_insert_size,
                                 std_insert_size = std_insert_size,
                                 threads = threads)
