...
def find_cliques_in_components(graph, 
    ...
    if len(small_row_indices) > 0:
        ...
        cliques_iter = gce_algorithm(selected_indices, small_weight_matrix, cutoff = ew_cutoff/3, logger = logger)
        for clique in cliques_iter:
            logger.info(f"Third round (edge weight cutoff = {ew_cutoff/3}): Receiving a clique containing {len(clique)} qnames in the small weight matrix")
