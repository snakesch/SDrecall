...
def is_umbrella_pair(pair1, pair2, coverage_threshold, overlap1, overlap2,
                     chrA, startA, endA, strandA, chrB, startB, endB, strandB):
    """
    Check if two pairs form an umbrella pair based on coverage and strand difference.
    ...
    if overlap1/overlap2 < 0.9:
        logger.info(f"The second pair ({pair2}) has bigger overlap with the target region interval than the first pair ({pair1})")
        return False
    ...
    result = coverage_A >= coverage_threshold and coverage_B >= coverage_threshold
