...
use crate::haplotype_determination::{
    determine_same_haplotype, get_error_vector, get_hap_vector, get_read_id, HaplotypeResult,
};
use crate::structs::{
    AlleleDepthMap, FastIntervals, HaplotypeConfig, OverlapInterval, PhasingGraphResult,
    ReadPairMap,
};
...
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│ • Initialize sparse num_nodes × num_nodes weight store     │
...
pub fn build_phasing_graph(
    ...
    if allele_depth_map.is_empty() {
        warn!(
            "[build_phasing_graph] No ALT allele found in this BAM file. Skip this entire script"
        );
    ...
    for qname_idx in 0..num_nodes {
        ...
        debug!(
            "[build_phasing_graph] Verified node correspondence: qname_idx {} = NodeIndex({})",
            qname_idx,
            node.index()
        );
    }
    ...
    for (&qname_idx, read_pair) in &read_pair_map.readpair_dict {
        ...
        let overlapping_qname_indices =
            get_overlapping_qname_indices_for_pair(read_pair_map, read_pair, &chrom)?;
        ...
        for other_qname_idx in overlapping_qname_indices {
            ...
            let edge_key = (
                qname_idx.min(other_qname_idx),
                qname_idx.max(other_qname_idx),
            );
            ...
            debug!(
                "[build_phasing_graph] Found {} overlap intervals between {} and {}",
                overlap_intervals.len(),
                qname,
                other_qname
            );
            ...
            for overlap in &overlap_intervals {
                ...
                let uncovered_regions =
                    find_uncovered_regions(&inspected_overlaps, overlap.start, overlap.end);

                debug!(
                    "[build_phasing_graph] Found {} uncovered regions for overlap {:?}",
                    uncovered_regions.len(),
                    overlap
                );

                for uncovered_region in uncovered_regions {
                    let (uncovered_start, uncovered_end) =
                        (uncovered_region[0], uncovered_region[1]);
    ...
    info!(
        "[build_phasing_graph] Finished building graph with {} vertices and {} edges",
        result.vertex_count(),
        result.edge_count()
    );
    info!(
        "[build_phasing_graph] Identified {} low-quality qnames during processing\n",
...
mod tests {
    ...
    fn uncovered_regions_independent_of_insertion_order() {
        ...
        assert_eq!(
            find_uncovered_regions(&intervals(&[(100, 150), (10, 50)]), 0, 200),
            want
        );
        assert_eq!(
            find_uncovered_regions(&intervals(&[(10, 50), (100, 150)]), 0, 200),
            want
        );
    }
