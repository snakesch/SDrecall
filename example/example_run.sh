...
SDRECALL_BIN=${SDRECALL_BIN:-"${PROJECT_DIR}/rust_modules/target/release/sdrecall"}
...
[[ -x ${SDRECALL_BIN} ]] || {
  echo "Rust SDrecall binary is missing: ${SDRECALL_BIN}"
  echo "Build it with: cargo build --release --manifest-path ${PROJECT_DIR}/rust_modules/Cargo.toml -p sdrecall"
  exit 1
}
...
MERGED_VCF="${OUTPUT_DIR}/HG002.hg38.CMRG.deepvariant.sdrecall.CMRGTEST.vcf.gz"
nohup "${SDRECALL_BIN}" \
--verbose DEBUG run \
...
--target_tag CMRGTEST \
--merged_vcf "${MERGED_VCF}" > "${OUTPUT_DIR}/test_SDrecall_CMRG_HG002.log" 2>&1 && \
echo "Test completed successfully. Result VCF: ${MERGED_VCF}" && \
echo "Intermediate files: ${OUTPUT_DIR}/HG002_hg38_CMRGTEST_SDrecall/" && \
echo "Added SDrecall variants are restricted to ${OUTPUT_DIR}/HG002_hg38_CMRGTEST_SDrecall/realign_groups/all_target_recall_SD_regions.bed" || \
