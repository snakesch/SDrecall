...
#     logger.info(f"****************** Step 2 completed in {(time.time() - start2) / 60} minutes. ******************")
    ...
    ### Optional: Merge variants called by SDrecall and other variant callers.
    if args.pvcf:
        ...
        elif not os.path.exists(ovcf):
            raise RuntimeError("Error in SDrecall. ")

        merge_all(args.pvcf, ovcf[0], args.output, args.pv_tag, "SDrecall", True)

    logger.info(f"****************** All processes completed in {time.time() - start0 // 60} minutes. ******************")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--build", type = str, required = True, choices = ["hg19", "hg38"], help = "reference genome assembly")
