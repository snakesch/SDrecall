...
# 2. Extract XA reads OR MQ < MQ_THRES
...
if [[ $(cat ${OUTPATH}/fastq/$(basename $INPUT_BAM | cut -d. -f1)_${REGION_PREFIX}-XA_1.fq.gz | wc -l) -lt 4 ]]
then
    ...
    rm -f ${OUTPATH}/*.tmp
    exit 0
elif [[ $(cat ${OUTPATH}/fastq/$(basename $INPUT_BAM | cut -d. -f1)_${REGION_PREFIX}-XA_2.fq.gz | wc -l) -lt 4 ]]
then
    echo -e >&2 "$(timestamp) WARNING: No multi-aligned reads for ${REGION_PREFIX} for $(basename $INPUT_BAM | cut -d. -f1)"
    rm -f ${OUTPATH}/fastq/$(basename $INPUT_BAM | cut -d. -f1)_${REGION_PREFIX}-XA_[1,2].fq
    rm -f ${OUTPATH}/*.tmp
